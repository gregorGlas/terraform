...
type MockTestRuns struct {
	...
	// ConfigurationVersions, uploadPaths, and uploadURLs keep track of our
	// tfe.ConfigurationVersion objects.
	ConfigurationVersions map[string]*tfe.ConfigurationVersion
	uploadPaths           map[string]string
	uploadURLs            map[string]*tfe.ConfigurationVersion
}
...
func newMockTestRuns(client *MockClient) *MockTestRuns {
	return &MockTestRuns{
		...
		logs:                  make(map[string]string),
		ConfigurationVersions: make(map[string]*tfe.ConfigurationVersion),
		uploadPaths:           make(map[string]string),
		uploadURLs:            make(map[string]*tfe.ConfigurationVersion),
	}
}

func (m *MockTestRuns) List(ctx context.Context, registryModuleId tfe.RegistryModuleID) (*tfe.TestRunList, error) {
	m.Lock()
	defer m.Unlock()
...
func (m *MockTestRuns) Read(ctx context.Context, testRunID string) (*tfe.TestRun, error) {
	m.Lock()
	defer m.Unlock()
	...
	if tr, exists := m.TestRuns[testRunID]; exists {
		...
		switch tr.Status {
		case tfe.TestRunStatusQueued:
			tr.Status = tfe.TestRunStatusRunning
		case tfe.TestRunStatusRunning:
			tr.Status = tfe.TestRunStatusComplete
		}
...
func (m *MockTestRuns) Create(ctx context.Context, options tfe.TestRunCreateOptions) (*tfe.TestRun, error) {
	...
	tr := &tfe.TestRun{
		ID:         id,
		LogReadURL: url,
		Status:     tfe.TestRunStatusQueued,
	...
	m.logs[tr.LogReadURL] = filepath.Join(
		m.uploadPaths[options.ConfigurationVersion.ID],
		"test.log",
	)
...
func (m *MockTestRuns) Logs(ctx context.Context, testRunID string) (io.Reader, error) {
	m.Lock()
	defer m.Unlock()
	...
	if err != nil {
		...
		case tfe.TestRunStatusComplete, tfe.TestRunStatusCancelled:
			// We're done.
			return true, nil
		...
		default:
			panic("unrecognized test status: " + string(tfe.TestRunStatusQueued))
		}
	}
...
func (m *MockTestRuns) Cancel(ctx context.Context, testRunID string) error {
	tr, exists := m.TestRuns[testRunID]
	if !exists {
		return tfe.ErrResourceNotFound
	}

	tr.Status = tfe.TestRunStatusCancelled
	return nil
}

func (m *MockTestRuns) ForceCancel(ctx context.Context, testRunID string) error {
	tr, exists := m.TestRuns[testRunID]
	if !exists {
		return tfe.ErrResourceNotFound
	}
...
func (m *MockTestRuns) CreateConfigurationVersion(ctx context.Context, registryModuleId tfe.RegistryModuleID) (*tfe.ConfigurationVersion, error) {
	m.Lock()
	defer m.Unlock()
	...
	m.ConfigurationVersions[id] = cv
	m.uploadURLs[url] = cv

	return cv, nil
}

func (m *MockTestRuns) UploadConfigurationVersion(ctx context.Context, url string, configurationPath string) error {
	m.Lock()
	defer m.Unlock()

	if cv, exists := m.uploadURLs[url]; exists {
		m.uploadPaths[cv.ID] = configurationPath
		cv.Status = tfe.ConfigurationUploaded
		return nil
	}
...
func (m *MockTestRuns) UploadConfigurationVersionTarGzip(ctx context.Context, url string, archive io.Reader) error {
	// We exclusively use the other function for uploading configuration so no
	// point implementing this one.
	panic("function not implemented in mock")
}
